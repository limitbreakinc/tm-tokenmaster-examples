...
abstract contract BondedPool is Ownable2Step, OwnableAccessControl, ERC20C, ITokenMasterERC20C {
    ...
    function _calculateShareSplit(uint256 totalShare) private view returns (
        ...
        unchecked {
            ...
            totalShare -= _creatorShareFeesPaidAdjustment;
    ...
    /**
     * @dev  Calculates amount of paired value required for an amount of tokens to buy, rounds up
     * @dev  to avoid underpayments of paired value.
     * 
     * @param tokensToBuy  Number of tokens being purchased.
     * @param numerator    Numerator to adjust tokens to buy to paired value.
     * @param denominator  Denominator to adjust tokens to buy to paired value.
     * 
     * @return pairedToPool  Value required in paired token for amount of tokens to buy.
     */
    function _calculatePairedToPool(uint256 tokensToBuy, uint256 numerator, uint256 denominator) internal pure returns (
        uint256 pairedToPool
    ) {
        uint256 product = tokensToBuy * numerator;
        pairedToPool = product / denominator;
        unchecked {
